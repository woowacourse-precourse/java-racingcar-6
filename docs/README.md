...
- [X] 사용자 입력 받기
    ...
    - [X] 전진을 시도할 횟수를 입력받는다.
- [X] 결과 출력
  - [X] 전진을 시도한 뒤 위치 정보를 출력한다. (위치 정보는 "-"의 개수로 출력)
  - [X] 전진 시도를 끝난 경우 우승자를 알려준다. (우승자는 한명 이상일 수 있다.)
  - [X] 2명 이상 우승자가 나올 경우 `pobi, jun(자동차 이름,{공백}자동차 이름)`와 같은 형태로 출력한다.
- [X] 입력받은 전진 시도 횟수만큼 자동차 경주를 반복한다.
- [X] 입력 받은 자동차 이름은 쉼표(,)를 기준으로 구분한다
- [X] 전진하는 조건은 랜덤으로 생성한 값이 4 이상인 경우 전진
- [X] 게임 마지막에는 '-'가 나온 총 합을 확인하여 가장 많이 전진한 자동차 이름을 우승자로 지정한다.
## 예외처리와 관련된 기능 목록
