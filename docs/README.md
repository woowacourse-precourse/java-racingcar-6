...
```
## Model
1. Player 객체 생성 - 각 플레이어별 이름과 움직인 횟수를 필드로 구성한다.
## View
1. getUserInfo() - (",")로 구분된 사용자 이름을 입력받을 수 있도록 한다.
2. getGameRound() - 사용자로부터 게임 진행횟수를 입력받는다.
3. printRoundInfo() - 게임 라운드마다 진행현황을 출력할 수 있도록 한다.
4. printWinner() - 우승자를 출력할 수 있도록 한다.
## Controller
사용자들을 관리하는 기능을 제공한다.
1. makeUsers() - 사용자를 생성할 수 있도록 한다.
2. removeUser() - 특정 유저를 삭제한다.
3. addUser() - 특정 유저를 추가한다.
## Logic
...
6. getWinsCount() - 사용자의 승리 횟수를 가져온다.
7. addWinsCount() - 사용자의 승리 횟수를 업데이트한다.
