...
- [x] 실행 결과를 이동 횟수만큼 출력한다. 출력: `실행 결과`
    - [x] 전진하는 자동차를 출력할 때 이름을 함께 출력하며 전진은 `-`로 표현한다. 출력: `$자동차 이름 : $전진 상태`
- [x] 경주 게임이 완료된 후 한 명 이상의 우승자를 알려준다.(우승자가 다수인 경우 `, `로 구분) 출력: `최종 우승자 : $자동차 이름`
