...
3) 받아온 랜덤값을 바탕으로 4이상일 시 각각의 자동차의 전진 회수 증가시키고 value값에 초기화한다.✅
...
### 5. Judgement
1) 자동차 이름과 전진 횟수를 담은 List<CarDto>를 받아 가장 많이 전진한 사용자를 문자열을 반환한다.✅
2) 우승자가 중복된 경우 리스트에 담에 반환한다.✅
...
3) "실행결과"를 출력한다.✅
4) 자동차 이름과 전진 횟수를 저장한 dto를 받아 경기당 누적 전진 횟수를 자동차 이름과 함께 출력한다.✅
5) 최종 우승자를 출력한다.✅
