...
* 경기에 참여할 자동차들의 이름을 입력 받는다.
* 총 라운드 수를 입력 받는다.
* n라운드를 진행한다.
    * `진행 결과`를 출력한다.
    * 한 라운드를 진행한다.
    * 한 라운드가 끝난 후 모든 자동차들의 전진 상태를 출력한다.
    * n번 반복한다.
* 최종 우승자를 출력한다.
...
* 자동차 정보 관리
    * **자동차 리스트**
        * 검증
            * null이면 안 됩니다.
            * 자동차 이름에 중복이 있으면 안 됩니다.
    * **하나의 자동차**
        * 전진 횟수
        * **자동차 이름**
            * 검증
                * null이면 안 됩니다.
                * 길이는 1자 이상 5자 이하여야 합니다.
                * 공백(` `, `\t`, `\n`) 포함되면 안 됩니다.
* 라운드 수 관리
    * **총 라운드 수**
        * 검증
    ...
    * 현재 라운드: 현재 진행되고 있는 라운드
        * 현재 라운드가 총 라운드 수와 같아질 때까지 경기가 진행되면 게임을 종료합니다. 이 때 쓰일 기능을 위해 필요.
