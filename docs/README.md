...
2. 커밋 주기를 메서드별 커밋에서 기능별 커밋으로 적용했다. 메서드별 커밋을 할 때 보다 커밋 하는 부분에 대해서 고민이 많이 줄어들었다. 기능별 커밋을 하더라도 메서드가 다른 메서드를 의존하지 않는다면 하나의 메서드가 곧 기능이 되기 때문에 메서드별 커밋을 할 때와 큰 차이가 없었고, 의존하는 경우 커밋 시점을 고민하지 않아도 되기 때문에 앞으로의 미션은 기능별 커밋을 적용하기로 했다.
