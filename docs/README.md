...
## 기능목록
[] 자동차를 생성하는 객체를 만든다. (Car.of / Car)
 - [] 자동차 이름은 ","로 구분하여 1글자이상 5글자이하의 영문이다. (CarName, CarId)
 - [] 자동차에 Id를 두어 동명의 자동차도 구분할 수 있게한다. (여기서 결과에 Id를 출력하지는 않는다.)
[] 자동차 객체들을 저장하는 객체(참가자 리스트)를 만든다. (Participants)

[] 첫 번째 이름입력값을 받아 유효성검사하고 변환한다. (InputHandler)
 - [] 변환한 입력값에 따라 자동차를 생성하고 저장한다. (CarSaveService.save / Participants)
...
## 테스트
[] 자동차 생성 및 첫번째 입력 테스트
 - [] 이름 각각은 대소문자 구분 없이 영문 1~5글자이다.
 - [] 이름엔 특수문자, 공백, 띄어쓰기가 포함될 수 없다.
[] 자동차 저장소 테스트
 - [] (기재되어 있지 않은사항) 참가 인원은 2명 이상으로 한다.
[] 레이스 테스트
