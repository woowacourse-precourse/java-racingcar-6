...
5. **테스트 코드 적용 방법**
   ...
   리팩터링을 진행하면서 부터는 test 코드 기능 구현 > main 기능 적용으로 변경하였습니다. 
   
   테스트 코드는 각각 모듈이 의미하는 기능을 inner static class로 정의했고, 각 기능들의 테스트가 끝나면

   테스트 코드의 inner static class들간의 기능을 가져와 통합 테스트를 진행하는 방식으로 했습니다..

   해당 방법이 좋은 방식일까요..? 아니면 더 좋은 방법이 있을까요 알려주시면 감사하겠습니다..ㅎㅎ