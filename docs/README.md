...
- [x] 이동 횟수 입력을 검증한다. `TriesCountInputValidator`
  - [x] 입력 값이 존재 하는가? 확인한다.
    - [x] 그 전에 공백을 제거한다.
  - [x] 정수인가? 확인한다.
  - [x] 양수인가? 확인한다.
...
- [x] IllegalArgumentException이 터질 때의 출력할 메세지를 정의한다. `ErrorMessage`
  - [x] 입력 값이 존재하지 않는 경우에 대한 정의
  - [x] 자동차 이름의 구분이 잘못된 경우에 대한 정의
  - [x] 이름에 중복이 있는 경우에 대한 정의
  - [x] 이름 규칙이 맞지 않는 경우(5자 이하여야 한다.)에 대한 정의
  - [x] 이동 횟수가 정수로 입력 되지 않은 경우에 대한 정의
...
- [x] 우승자를 선정한다. `WinnerDeterminationService`
  - [x] 총 시도 횟수 동안 가장 많은 전진을 한 자동차를 찾아 정의한다.
  - [x] 우승자는 중복을 허용한다.
