...
### 테스트
- [x] 1️⃣입출력 관련
  - [x] 시도 횟수 입력 시 숫자가 아닌 경우 IllegalArgumentException 이 던져지는지
  - [x] 구분자를 기준으로 문자열을 자르고 앞 뒤 공백을 제거해서 리스트로 반환하는지
- [x] 2️⃣ 자동차 등록 관련
  - [x] 쉼표를 기준으로 문자열이 잘려서 자동차 이름이 등록되는지
  - [x] 이름이 5글자가 넘어가면 IllegalArgumentException 이 던져지는지
- [ ] 3️⃣ 자동차 이동 관련
  - [ ] 무작위 값이 4 이상인 경우 전진하는지
  - [ ] 무작위 값이 4보다 작은 경우 전진하지 않는지
