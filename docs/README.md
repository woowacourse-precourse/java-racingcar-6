...
- [ ] `move` 메서드를 생성한다.
- [ ] 게임이 바뀔 수도 있을 것에 대비해 `RacingGame`의 `move` 메서드를 상속받도록 구현한다.
    - [ ] RacingGame을 인터페이스로 만든다. 
#### Car
자동차의 대한 정보를 저장하고 자동차의 관련된 기능을 담당하는 객체이다.

- [ ] 자동차의 이름을 저장할 `name`필드를 생성한다.
    - 예외처리
    - [ ] 자동차의 이름이 5글자 이하인지 검증한다. 
- [ ] `canMove` 메서드를 통해 자동차가 전진을 할 수 있는지 확인한다.
  - 전진 할 수 있는 조건
    - 0~9까지의 랜덤 숫자를 만들고 이 숫자가 4 이상일 때 전진 가능
- [ ] 자동차가 다른 탈 것으로 바뀔 것을 대비하여 `Vehicle`의 `canMove`메서드를 상속 받도록 구현한다.
  - [ ] `Vehicle`은 인터페이스로 만든다.
