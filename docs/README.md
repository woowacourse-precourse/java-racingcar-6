...
- 기능 목록
    ...
    - ReadRacingResult
        - [ ] 각 전진 세트당 현황 출력
            ...
            - [ ] 정수를 "-" 의 갯수로 반환해주는 기능 - toDash()
            - [ ] 경주 현황 기록 해시맵 키값(cars[]내 자동차이름들) 을 하나씩 읽기 - for 활용
            - [ ] 하나씩 읽어들인 자동차 이름을 키값으로 해서 자동차 이동거리 받아오기 - get 활용
            - [ ] 받아들인 결과값을 토대로 문자열 생성 -

        - [ ] 자동차 경주 게임을 완료한 후 누가 우승했는지를 알려준다. 우승자는 한 명 이상일 수 있다. - JudgmentWinner()
            - [ ] resultRacing를 토대로 경주 완료 판단 -
            - [ ] 우승자 확인 (1명이상 가능)

        - [ ] 자동차 이름은 쉼표(,)를 기준으로 구분하며 이름은 5자 이하만 가능하다. - HandlingString()
            - [ ] 자동차 이름을 쉼표(,) 기준으로 구분
            - [ ] 이름이 5자 이하인가 검사
