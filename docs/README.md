...
|---|---|---|------|---|---|
|게임 실행하기|입력, 출력, 차 움직이기에 있는 메소드를 이용해 게임을 시작한다|startRacing| 0    ||#13|
...
|실행 결과 문구 출력하기|"실행 결과"를 출력한다|putResultStatement| 0     ||#11|
|현황 출력하기|한 번 차가 움직인 결과를 출력한다|putResultOfMovement| 0     || #05   |
|각 차의 위치 출력하기|각 차가 움직인 결과를 출력한다|putResultOfEachCar| 0     || #10   |
|최종 우승자 구하기|최종 우승자들의 이름을 구한다|getResultOfWinner| 0     ||#06|
|최종 우승자 출력하기|최종 우승자들의 이름을 출력한다|putResultOfWinner| 0     ||#12|
|입력 닫기|입력을 비운 후 닫는다|closeInput| 0     || #09   |
...
# 테스트 케이스 정리
- [ ] Car를 이동시키는 메소드들의 검증
  - [ ] n개의 true || false가 담긴 Boolean 배열이 만들어지는가
  - [ ] boolean 배열의 값에 따라 Car들이 담긴 배열의 값이 정상적으로 바뀌는가

- [ ] 입력을 받는 메소드들의 검증
  - [ ] 차 이름이 5자리가 넘는가에 대한 검증
  - [ ] 횟수에 문자가 들어올 때의 예외 검증
...
- [ ] 큰 규모의 입력에 대한 출력 검증
  - [ ] 큰 규모의 입력에 대한 출력 검증 (1)
  - [ ] 큰 규모의 입력에 대한 출력 검증 (2)
---
# 고려한 점 && 피드백 반영 사항
---
