...
public class TestConstantsProvider {
    ...
    public static Stream<Arguments> provideInvalidSystemTestPlayNumberConstants() {
        return Stream.of(
                arguments(ONE_CAR_NAME, named("null", NULL_STRING)),
                arguments(ONE_CAR_NAME, named("빈 문자열", EMPTY_STRING)),
                arguments(ONE_CAR_NAME, named("공백 문자열", BLANK_STRING)),
                arguments(ONE_CAR_NAME, named("0", PLAY_NO_TIME)),
                arguments(ONE_CAR_NAME, named("0보다 작은 값", PLAY_NEGATIVE_TIME)),
                arguments(ONE_CAR_NAME, named("숫자가 아닌 값", NOT_NUMBER_STRING))
        );
    }
