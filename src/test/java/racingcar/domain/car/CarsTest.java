...
class CarsTest {
    ...
    void Cars_move_정상작동_2개다_전진() {
        ...
        when(driveRule.generateValue(MIN_RANGE, MAX_RANGE)).thenReturn(4, 4);
    ...
    void Cars_move_정상작동_전진_못함() {
        DriveRule driveRule = mock(RandomDriveRule.class);
        when(driveRule.generateValue(MIN_RANGE, MAX_RANGE)).thenReturn(2, 2);
    ...
    void Cars_getWinner_중복된_승자_모두_반환() {
        DriveRule driveRule = mock(RandomDriveRule.class);
        when(driveRule.generateValue(MIN_RANGE, MAX_RANGE)).thenReturn(5, 5);
