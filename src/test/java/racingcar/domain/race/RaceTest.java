...
class RaceTest {
    ...
    void 경주는_정상적으로_진행_모두_진행() {
        ...
        when(driveRule.generateValue(MIN_RANGE, MAX_RANGE)).thenReturn(4);

        Cars cars = CarsFixture.fixture().build();

        Race race = new Race(driveRule);
    ...
    void 경주는_정상적으로_진행_2개만_진행() {
        DriveRule driveRule = mock(RandomDriveRule.class);
        when(driveRule.generateValue(MIN_RANGE, MAX_RANGE)).thenReturn(4, 4, 2);
    ...
    void 경주는_정상적으로_진행_1개만_진행() {
        DriveRule driveRule = mock(RandomDriveRule.class);
        when(driveRule.generateValue(MIN_RANGE, MAX_RANGE)).thenReturn(2, 4, 2);
    ...
    void 경주는_정상적으로_진행_진행_못함() {
        DriveRule driveRule = mock(RandomDriveRule.class);
        when(driveRule.generateValue(MIN_RANGE, MAX_RANGE)).thenReturn(2, 2, 2);
