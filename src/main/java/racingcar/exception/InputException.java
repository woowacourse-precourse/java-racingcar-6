package racingcar.exception;

import java.util.List;

public class InputException {
    public static void emptyException(String input) {
        if (input.isEmpty()) {
            throw new IllegalArgumentException("빈 값이 입력 되었습니다.");
        }
    }

    public static void outOfNameLengthException(String input) {
        if (input.length() > 5) {
            throw new IllegalArgumentException("5자 이상의 이름이 입력 되었습니다.");
        }
    }

    public static void outOfRangeException(String input) {
        for (int i = 0; i < input.length(); i++) {
            if (input.charAt(i) < '0' || input.charAt(i) > '9') {
                throw new IllegalArgumentException("범위에 벗어난 값이 입력 되었습니다.");
            }
        }
    }

    public static void existsNameException() {
        throw new IllegalArgumentException("중복되는 이름이 포함되어 있습니다.");
    }
}
