...
public class GameManagerUtils {
    ...
    //우승자 이름이 담겨있는 리스트 반환
    public static List<String> winnerCarName(int winnerCount, List<Integer> forwardCount, List<String> cars){
    ...
    public static int randomNumber(List<String> cars){
    ...
    public static List<Integer> plusForwardCount(List<Integer> forwardCount, List<String> cars){
        for(int i = 0; i < forwardCount.size(); i++){
            if(randomNumber(cars) >= MIN_FORWARD_THRESHOLD) forwardCount.set(i, forwardCount.get(i) + 1);
        }
        return forwardCount;
    }
    //전진 수만큼 "-" 채우기
