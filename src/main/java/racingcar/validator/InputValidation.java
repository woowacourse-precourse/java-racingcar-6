...
public class InputValidation {
    public static void validNameFormat(String name) throws IllegalArgumentException{
        String pattern = "^(?:[^,]+,)*[^,]+$";
        if (!Pattern.matches(pattern, name))    throw new IllegalArgumentException(VALIDATE_INPUT_ERROR_MESSAGE.getValue());
    }
    public static void validNameLength(String name) throws IllegalArgumentException{
        if (name.length() > 5) throw new IllegalArgumentException(VALIDATE_NAME_LENGTH_ERROR_MESSAGE.getValue());
    }
}
