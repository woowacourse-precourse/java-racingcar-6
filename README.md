...
## 구현할 기능 목록
- 경주 할 자동차들의 이름을 입력받기 (5글자 이하의 이름을 쉼표로 구분)
- 경주 게임을 시도할 회수를 입력받기
- 랜덤한 값으로 각 시도마다 자동차 전진(0~9 사이의 무작위 값중 4이상일 경우 전진)
- 자동차의 이름과 각각 차수별 실행 결과를 표시하기
- 경기가 종료되면 우승자를 발표, 우승자가 여러명일 경우 쉼표로 구분하기
- 사용자가 잘못된 값을 입력할 경우 IllegalArgumentException 을 던져 어플리케이션 종료
