...
## 📋 구현 기능 목록
- ### 차 클래스
- [x] 이름을 멤버 변수로 가짐, 5자 이하
- [x] 전진 횟수를 멤버 변수로 가짐
- [x] 전진하는 메소드가 있음, 전진할 때 이름도 같이 출력
- [x] 전진할지 말지 결정하는 메소드가 있음, 0 ~ 9 사이 무작위 숫자가 4 이상이면 전진

- ### 게임 클래스
- [x] 게임에 참여한 차 리스트를 멤버 변수로 가짐
- [x] 몇 번의 이동을 할 지 입력받은 값을 멤버 변수로 가짐
- [x] 게임이 종료되었는지 확인하는 메소드가 있음
- [x] 게임을 한번 시도하는 메소드가 있음

- [x] 사용자가 잘못된 값을 입력할 경우 IllegalArgumentException 발생
  - [x] 시도 횟수를 0 이상의 숫자로 입력하지 않은 경우
  - [x] 이름의 길이가 0 이하, 6 이상일 경우
