...
### 경주 진행
- 각 회차별로 다음을 수행:
  - [x] 각 자동차마다 0에서 9 사이의 난수를 생성하고, 생성된 값이 4 이상이면 전진한다.
  - [x] 전진한 자동차의 이름을 출력하고, 각 자동차의 진행 상태를 출력한다.

### 경주 결과 계산
- [x] 모든 회차가 종료되면 우승자를 결정한다.
- [x] 우승자는 가장 많은 전진 횟수를 가진 자동차 또는 여러 대의 자동차가 동시에 우승할 수 있다.

### 우승자 출력
- [x] 우승자(들)의 이름을 출력한다.
- [x] "최종 우승자" 문구와 함께 출력한다.
- [x] 공동 우승자가 있는 경우, 쉼표로 구분하여 출력한다.

### 사용자 오류 처리
- [x] 사용자가 잘못된 값을 입력한 경우, IllegalArgumentException을 발생시키고 애플리케이션을 종료한다.