...
## 기능 목록
- [ ] 자동차 이름을 입력받기.
- [ ] 입력받은 자동차 이름을 쉼표(,)를 기준으로 구분하여 자동차 리스트 생성 (차 이름이 5글자 이하일때).
- [ ] 사용자로부터 시도할 회수 입력 받기.
- [ ] 각 시도마다 랜덤 숫자를 생성하여 자동차를 전진 또는 멈추게 하기.
- [ ] 자동차 전진 조건은 0에서 9까지의 랜덤 숫자 중 4 이상일 경우에 동작한다.
- [ ] 시도가 끝날 때마다 각 자동차의 위치를 출력한다.
- [ ] 경주가 끝난 후 우승자를 출력한다.
- [ ] 잘못된 입력값에 대한 예외 처리를 한다.
