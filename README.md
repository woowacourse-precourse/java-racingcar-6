...
- Player
	- 사용자는 각 자동차에 이름을 부여한다.(5자 이하, 쉼표로 구분)
	- 사용자는 몇 번 이동할 것인지 입력한다.
- Car
	- 컴퓨터는 각 자동차마다 0에서 9 사이의 무작위 값을 구한다.
	- 무작위 값이 4 이상인 경우 전진(미만인 경우 정지)
	- 사용자가 입력한 횟수만큼 위의 경우를 반복한다
- 출력
	- 시도 횟수가 끝나면 경주 게임에서 우승자를 알려준다. (우승자는 한 명 이상일 수 있다, 쉼표로 구분)
	- 전진하는 자동차를 출력할 때 자동차 이름을 같이 출력한다.

- 예외
	- 사용자가 잘못된 값을 입력할 경우 `IllegalArgumentException`을 발생시킨 후 애플리케이션은 종료되어야 한다.
		- 자동차를 등록하지 않거나 하나일 경우
		- 자동차의 이름이 5자를 초과할 경우
		- 이동횟수에 숫자가 아닌 문자를 입력할 경우
