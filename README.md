...
## 구현할 기능 목록
- 이름, 거리를 저장하는 Car 클래스를 정의한다.
- Car 클래스의 리스트를 생성한다.
- 이름을 입력받는다. 
- 쉼표를 기준으로 구분하여 저장하고 거리를 초기화한다.
- 시도 횟수 입력받는다.
- 이름이 6자 이상인 경우, 즉 잘못된 값이 있는지 판단한다.
- 잘못된 값이 있으면 애플리케이션을 종료한다.
- 시도 횟수만큼 반복한다.
- 무작위 값을 구한다.
- 무작위 값이 조건에 부합하는 지 판단한다.
- 조건에 부합하면 해당 자동차를 전진한 상태로 갱신한다.
- 가장 많이 전진한 자동차의 거리를 확인한다.
- 가장 많이 전진한 거리와 같은 자동차를 저장한다.
- 정답 목록을 출력한다.